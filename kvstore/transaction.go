...
func (kv *KV) Begin(tx *KVTX) {
	...
	pages := [][]byte(nil)
	tx.pending.GetPage = func(ptr uint64) []byte { return pages[ptr-1] }
	tx.pending.NewPage = func(node []byte) uint64 {
	...
	runtime.SetFinalizer(tx, func(tx *KVTX) { utils.Assert(tx.done) })
}
...
func (kv *KV) Commit(tx *KVTX) error {
	...
	for iter := tx.pending.Seek(nil, btree.CMP_GT); iter.Valid(); iter.Next() {
		...
		case FLAG_DELETED:
			modified = isOld
			deleted, err := kv.tree.Delete(&btree.DeleteReq{Key: key})
			utils.Assert(err == nil)          // can only fail by length limit
			utils.Assert(deleted == modified) // assured by conflict detection
		case FLAG_UPDATED:
			modified = (!isOld || !bytes.Equal(oldVal, val[1:]))
			updated, err := kv.tree.Update(&btree.UpdateReq{Key: key, Val: val[1:]})
			utils.Assert(err == nil)          // can only fail by length limit
			utils.Assert(updated == modified) // assured by conflict detection
		default:
			panic("unreachable")
		}
...
func (tx *KVTX) Seek(key1 []byte, cmp1 int, key2 []byte, cmp2 int) KVIter {
	utils.Assert(cmp2dir(cmp1) != cmp2dir(cmp2))
	lo, hi := key1, key2
	if cmp2dir(cmp1) < 0 {
		lo, hi = hi, lo
	}
...
// capture updates
func (tx *KVTX) Update(req *btree.UpdateReq) (bool, error) {
	...
	_, err := tx.pending.Update(&btree.UpdateReq{Key: req.Key, Val: flaggedVal})
	if err != nil {
...
func (tx *KVTX) Del(req *btree.DeleteReq) (bool, error) {
	tx.updateAttempted = true
	exists := false
	if req.Old, exists = tx.Get(req.Key); !exists {
		return false, nil
	}
	return tx.pending.Update(&btree.UpdateReq{Key: req.Key, Val: []byte{FLAG_DELETED}})
}
