...
func (r *R) add(table string, rec Record) bool {
	...
	utils.Assert((idx < 0) == dbreq.Added)
	if idx < 0 {
...
func (r *R) get(table string, rec *Record) bool {
	...
	if ok {
		utils.Assert(idx >= 0)
		records := r.ref[table]
		utils.Assert(reflect.DeepEqual(records[idx], *rec))
	} else {
		utils.Assert(idx < 0)
	}
	return ok
...
func TestTableEncoding(t *testing.T) {
	input := []int64{-1, 0, +1, math.MinInt64, math.MaxInt64}
	slices.Sort(input)

	encoded := []string{}
	for _, i := range input {
		v := Value{Type: TYPE_INT64, I64: i}
		b := encodeValues(nil, []Value{v})
		out := []Value{v}
		decodeValues(b, out)
		utils.Assert(out[0].I64 == i)
...
func TestTableScan(t *testing.T) {
	...
	tx := r.begin()
	{
		rec := Record{} // empty
		req := Scanner{
			Cmp1: btree.CMP_GE, Cmp2: btree.CMP_LE,
			Key1: rec, Key2: rec,
		}
...
func TestTableIndex(t *testing.T) {
	...
	tx = r.begin()
	{
		...
		rec2.AddInt64("i2", 4)
		req := Scanner{
			Cmp1: btree.CMP_GT, Cmp2: btree.CMP_LE,
			Key1: rec1, Key2: rec2,
		}
		err := tx.Scan("tbl_test", &req)
		utils.Assert(err == nil)
