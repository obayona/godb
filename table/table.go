...
func (tx *DBTX) Save(save *kvstore.TXSave) {
	tx.kv.Save(save)
}
func (tx *DBTX) Revert(save *kvstore.TXSave) {
	tx.kv.Revert(save)
}
...
func decodeValues(in []byte, out []Value) {
	for i := range out {
		utils.Assert(out[i].Type == uint32(in[0]))
		in = in[1:]
		switch out[i].Type {
		case TYPE_INT64:
...
func checkIndexCols(tdef *TableDef, index []string) ([]string, error) {
	...
	utils.Assert(len(index) <= len(tdef.Cols))
	return index, nil
}
...
func (tx *DBTX) TableNew(tdef *TableDef) error {
	...
	if ok {
		...
		utils.Assert(prefix > TABLE_PREFIX_MIN)
	} else {
		meta.AddStr("val", make([]byte, 4))
	}
	utils.Assert(len(tdef.Prefixes) == 0)
...
func dbUpdate(tx *DBTX, tdef *TableDef, dbreq *DBUpdateReq) (bool, error) {
	...
	if req.Updated && !req.Added {
		...
		utils.Assert(err == nil) // should not run into the length limit
	}
	if req.Updated {
...
func indexOp(tx *DBTX, tdef *TableDef, op int, rec Record) error {
	for i := 1; i < len(tdef.Indexes); i++ {
		...
		case INDEX_ADD:
			...
			utils.Assert(req.Added) // internal consistency
		case INDEX_DEL:
			deleted, err := tx.kv.Del(&btree.DeleteReq{Key: key})
			utils.Assert(err == nil) // should not run into the length limit
			utils.Assert(deleted)    // internal consistency
		default:
			panic("unreachable")
		}
	}
...
func dbDelete(tx *DBTX, tdef *TableDef, rec Record) (bool, error) {
	...
	// delete the row
	req := btree.DeleteReq{Key: encodeKey(nil, tdef.Prefixes[0], values)}
...
type Scanner struct {
	...
	iter  kvstore.KVIter // the underlying kvstore.KV iterator
}
...
func (sc *Scanner) Deref(rec *Record) {
	...
	// fetch the kvstore.KV from the iterator
	...
	if sc.index == 0 {
		...
		utils.Assert(ok && err == nil) // internal consistency
	}
}
