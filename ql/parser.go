...
func pColType(p *Parser) uint32 {
	...
	case "string", "bytes":
		return table.TYPE_BYTES
	case "int", "int64":
		return table.TYPE_INT64
	default:
...
func pSelectExpr(p *Parser, node *QLSelect) {
	if pKeyword(p, "*") {
		...
		node.Output = append(node.Output, QLNode{Value: table.Value{Type: QL_STAR}})
		return
	}
...
func pExprBinop(
	...
	utils.Assert(len(ops) == len(types))
	left := QLNode{}
	next(p, &left)
	...
	for {
		...
		new := QLNode{Value: table.Value{Type: types[i]}}
